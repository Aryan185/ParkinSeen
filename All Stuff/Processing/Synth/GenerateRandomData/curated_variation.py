...
# Add randomness around the mean values with specified standard deviations
std_left_x = 36  # Standard deviation for Left_X
std_left_y = 153  # Standard deviation for Left_Y
synthetic_left_x += np.random.normal(scale=std_left_x, size=num_rows)
synthetic_left_y += np.random.normal(scale=std_left_y, size=num_rows)
...
synthetic_data.to_csv("curated_synthetic_data.csv", index=False)
